...
function entryPath(namespace: string, cacheId: string) {
...
function fail(message: string, details?: any, code = 1) {
...
async function main() {
  ...
  if (action === "stats") {
    ...
    for (const f of files) {
      ...
      if (entry && entry.expires_at && Date.now() > new Date(entry.expires_at).getTime()) expired++;
    }
  ...
  if (action === "lookup") {
    const entry = (await safeReadJson(p)) as CacheEntry | null;
    if (!entry) return ok({ cache_hit: false });
