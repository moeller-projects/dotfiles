...
function entryPath(ctx, namespace, cacheId) {
...
export async function analysisCacheLookup(input, ctx) {
...
export async function analysisCacheStore(input, ctx) {
    const namespace = sanitizeSegment(input.namespace ?? "default");
    const key = requireKey(input.key);
    const keyExtra = input.key_extra ?? null;
...
export async function analysisCacheInvalidate(input, ctx) {
    const namespace = sanitizeSegment(input.namespace ?? "default");
    const root = resolveCacheRoot(ctx);
...
export async function analysisCacheStats(input, ctx) {
    const namespace = sanitizeSegment(input.namespace ?? "default");
    const root = resolveCacheRoot(ctx);
    const nsDir = path.join(root, namespace);
    const files = await listFilesRecursive(nsDir);
