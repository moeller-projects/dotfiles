...
// Keep hook parameter types conservative to avoid depending on internal OpenCode types
type ToolExecuteBeforeInput = { tool?: string } | unknown;
type ToolExecuteBeforeOutput = { args?: Record<string, unknown> } | unknown;

export const ControlPlane: Plugin = async ({ client }) => {
  ...
  return {
    ...
    "tool.execute.before": async (input: ToolExecuteBeforeInput, output: ToolExecuteBeforeOutput) => {
      ...
      const patch = (output as any)?.args?.patch;
      if (typeof patch !== "string" || patch.length === 0) return;

      const max = Number((output as any)?.args?.max_change_percent ?? 30);
      const strict = Boolean((output as any)?.args?.strict ?? true);
