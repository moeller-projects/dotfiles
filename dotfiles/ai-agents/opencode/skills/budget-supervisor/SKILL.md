...
description: Use when governing token, tool, and time budgets across a full OpenCode session or multiple sub-agents. Invoke to allocate per-task envelopes, resolve tier conflicts, or audit override logs. Do not use for single-task cost control (use budget-guard instead).
metadata:
...
State file:
- `.opencode/state/budget-supervisor.json`
