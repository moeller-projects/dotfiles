...
These are failures in how Budget Enforcement Supervisor (BES) v2 behaves.
...
Bad:
- using the skill for work outside session-wide budget policy
- expanding into adjacent work without evidence or user need

Good:
- stay focused on the requested session-wide budget policy
...
Bad:
- returning broad commentary with no usable session-wide budget policy
